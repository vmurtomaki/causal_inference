...
function Get-Tree($CurrentDir, $Indent) {
    # Tailored exclusions for causal_inference
    $ExcludeDirs = @('.git', '.venv', '__pycache__', 'data', '.mypy_cache', '.pytest_cache', '.ruff_cache')
...
Write-Host "Scanning causal_inference structure..." -ForegroundColor Cyan
